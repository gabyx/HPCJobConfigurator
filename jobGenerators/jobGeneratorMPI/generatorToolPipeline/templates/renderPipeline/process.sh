...
if [ ! -f "$fileMoverProcessFile" ]; then
    ...
    begin=$(date +"%s")
    try python -m JobGenerator.jobGenerators.jobGeneratorMPI.generatorToolPipeline.scripts.fileMove \
...
if [ ! -f "$renderProcessFile" ]; then
    ...
    try python -m JobGenerator.jobGenerators.jobGeneratorMPI.generatorToolPipeline.scripts.renderPipeline.renderFrames \
        -p "$renderProcessFile" \
