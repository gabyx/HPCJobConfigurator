...
from JobGenerator.jobGenerators.dictionaryAdjuster import DictionaryAdjuster
            ...
            for k,templ in self.config["Templates"].items():
                if k not in self.config["TemplatesOut"]:
                    ...
                    if not os.path.exists(templ): # this is no valid file
                        raise ValueError("You did not specify an output file for option %s in section TemplatesOut! and template is no existing file!" % k)
