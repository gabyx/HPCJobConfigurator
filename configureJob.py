...
generatorsModuleDir = os.path.abspath(os.path.join(os.path.dirname(
                                    inspect.getfile(inspect.currentframe())
                                    )
                                  ))
jobGeneratorModuleDir = os.path.abspath(os.path.join(generatorsModuleDir,".."))

sys.path.append( jobGeneratorModuleDir )
...
def submitInternal(config):
    ...
    config["General"] = {
                         "modulePathJobGen" : jobGeneratorModuleDir,
                         "modulePathGenerators" : generatorsModuleDir,
    ...
    comps = config.Cluster.jobGenerator.split(".")
    className =  comps[-1]
    moduleName = ".".join(comps[0:-1])
